  ...
  return (
    <footer className="fixed bottom-0 left-0 z-10 flex w-full select-none items-center justify-between px-4 py-2 font-monument font-extrabold text-stone-700">
      ...
      <div className="flex flex-col text-end">
        <p className="flex-1 text-[0.5rem] capitalize">Environment: {process.env.NEXT_PUBLIC_NODE_ENV || "n/a"}</p>
        <p className="flex-1 text-[0.5rem]">Build: {process.env.NEXT_PUBLIC_BUILD_NUMBER || "n/a"}</p>
      </div>
    </footer>
  );
};
